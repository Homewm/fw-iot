...
class TISpider(Spider):
    ...
    def parse_link(self, response):
        ...
        for link in response.xpath("//tbody[@class='etdownloaditems']//tr"):
            ...
            version = link.xpath(".//td[@class='column-version']//text()").extract()[0].strip()
            url = link.xpath(".//th/a/@href").extract()[0]
            absurl = urlparse.urljoin(response.url,url)
            ...
            item["description"] = link.xpath(".//th/a//text()").extract()[0]
            item["url"] = absurl
